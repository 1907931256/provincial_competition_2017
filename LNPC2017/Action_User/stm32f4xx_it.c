...
extern int startReceive;
extern int endReceive;
extern int ballAmount;
extern int ball[50][2];	
static int i = 0, j = 0;
void USART2_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART2, USART_IT_RXNE) == SET)
	{
		USART_ClearITPendingBit(USART2, USART_IT_RXNE);
				if(startReceive != 0xD6 && endReceive == 0xD5)
		{
			startReceive = USART_ReceiveData(UART5);
			if(startReceive == 0xD6)
			{
				endReceive = 0;
				for(j = 0; j < 50; j++)
				{
		...
		else if(startReceive == 0xD6 && endReceive != 0xD5)
		{
			if(USART_ReceiveData(UART5) != 0xD6 && USART_ReceiveData(UART5) != 0xD5)
			{	
				ball[i][j] = (int8_t)USART_ReceiveData(UART5);
				j++;
				if(j == 2)
				{
					j = 0;
			...
			else if(USART_ReceiveData(UART5) == 0xD6)
			{
				for(j = 0; j < 50; j++)
				{
					ball[j][0] = 0;
			...
			else if(USART_ReceiveData(UART5) == 0xD5)
			{
				endReceive = 0xD5;
				startReceive = 0;
				ballAmount = i;
...
//定位系统
extern float dirX , dirY , dirAngle;
...
void USART3_IRQHandler(void) //更新频率200Hz
{
	...
	if (USART_GetITStatus(USART3, USART_IT_RXNE) == SET)
	{
		...
		case 4:
			if (ch == 0x0d)
			{
				posture.ActVal[0] = posture.ActVal[0];//角度			
				posture.ActVal[1] = posture.ActVal[1];
				posture.ActVal[2] = posture.ActVal[2];
				posture.ActVal[3] = posture.ActVal[3];//x
				posture.ActVal[4] = posture.ActVal[4];//y
				posture.ActVal[5] = posture.ActVal[5];
				fakeAngle = posture.ActVal[0];
				fakeX = posture.ActVal[3];
				fakeY = posture.ActVal[4];
				x = posture.ActVal[3] * cosf(dirAngle / 180.0f * 3.141592f) + posture.ActVal[4] * sinf(dirAngle / 180.0f * 3.141592f) - dirX;
				y = -posture.ActVal[3] * sinf(dirAngle / 180.0f * 3.141592f) + posture.ActVal[4] * cosf(dirAngle / 180.0f * 3.141592f) - dirY;
				angle = posture.ActVal[0] - dirAngle;
				if(angle < -180.0f)
				{
					angle += 360.0f;
				}
